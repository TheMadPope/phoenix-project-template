...
SCRATCH_ORG_ALIAS="scratch"

# Alias for the SANDBOX used by this developer.
SCRATCH_ORG_ALIAS="scratch"
...
if [ "$ECHO_LOCAL_CONFIG_VARS" = "true" ]; then
  echo "`tput setaf 7``tput bold`Local configuration variables set by `dirname $0`/lib/local-config.sh`tput sgr0`"
  echoConfigVariables
fi
##END##